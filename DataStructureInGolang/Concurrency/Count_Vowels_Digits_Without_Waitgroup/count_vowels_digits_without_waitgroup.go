...
func main() {
	...
	vowelsCh   := make(chan bool)
	digitsCh   := make(chan bool)

	//wg.Add(2)

	go countVowelsFunc(countVowels,userInput,vowelsCh)
	go countDigitsFunc(countDigits,userInput,digitsCh)
...
func countVowelsFunc(input chan int, str string, flag chan bool){
...
func countDigitsFunc(input chan int, str string,flag chan bool){
	var digitCount int
